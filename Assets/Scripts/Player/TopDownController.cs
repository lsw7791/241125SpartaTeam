...
public class TopDownController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        _unitRoot = GetComponentInChildren<UnitRoot>();
        camera = Camera.main;
    }
    ...
    private void FlipRotation(Vector2 mouseWorldPos)
    {
        ...
        {
            // 왼쪽을 바라보게 (회전)
            _unitRoot.transform.rotation = Quaternion.Euler(0, 0, 0);  // Y축 회전 180도
        }
        else
        {
            // 오른쪽을 바라보게 (회전 취소)
            _unitRoot.transform.rotation = Quaternion.Euler(0, 180, 0);    // 기본 회전 (Y축 0도)
        }
    }
